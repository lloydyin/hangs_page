...
def get_place_name(file_name):
    place_name = Path(file_name).stem
    while Path(place_name).suffix:
        place_name = Path(place_name).stem
    return place_name
...
def generate_photo_data(folder_path):
    ...
    for full_path in sorted(Path(folder_path).iterdir(), key=lambda path: path.name.lower()):
        if full_path.suffix.lower() in IMAGE_EXTENSIONS:
            ...
            data = {
                "file": f"images/photosphere/{file_name}",
                "lat": latitude,
                "lng": longitude,
                "place": get_place_name(file_name),
                "date": date,
                "datePrecision": "month"
            }
...
def main():
    ...
    output_path = folder_path / "photosphere.json"
